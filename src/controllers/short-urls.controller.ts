...
export default class ShortUrlsController implements Controller {
    ...
    constructor(
        private shortsUrlsService: ShortUrlsService
    ) {
    ...
    getShortUrls(req: Request, res: Response): void {
        this.shortsUrlsService.getShortUrls()
        .then(response => res.status(200).json(response))
        .catch(error => res.status(500).json(error));
    }

    getUrlByShortId(req: Request, res: Response): void {
        this.shortsUrlsService.getUrlByShortId(req.params.shortId)
        .then(response => res.status(200).json(response))
        .catch(error => res.status(500).json(error));
    }

    createShortUrl(req: Request, res: Response): void {
        this.shortsUrlsService.createShortUrl(req.body)
        .then(response => res.status(200).json(response))
        .catch(error => res.status(500).json(error));
    }

    deleteShortUrl(req: Request, res: Response): void {
        this.shortsUrlsService.deleteShortUrl(req.params.shortId)
        .then(response => res.status(200).json(response))
        .catch(error => res.status(500).json(error));
    }
